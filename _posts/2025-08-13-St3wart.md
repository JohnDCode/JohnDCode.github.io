...
```
Note: The end vision for St3wart is for an IT Administrator to be able to remotely check for any custom defined vulnerability
 on any one of their systems, and then automatically deploy proper actions to secure the vulnerability if necessary. The current 
 implementation of St3wart is specifically made for Windows and does not include remote scanning. Subsequent versions of St3wart 
 will implement these features.
```
